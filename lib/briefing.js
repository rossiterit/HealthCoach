...
function assemble(store, cfg, now = new Date()) {
  ...
  // v3 F4: today's plan, when there is one. This adds a LINE to the existing
  // single send — never a message. The one-touch rule is untouched.
  // plannedLine() returns null on a Sunday by itself (Decision 3), so the free
  // day carries no plan line and nothing here has to remember that.
  const plannedLine = planner.plannedLine(store, cfg, today);
...
3. YESTERDAY'S FOOD, ONE LINE — ${f.foodLine}${
    f.plannedLine
      ? `\n   Then, in the same breath, what they have PLANNED for today: ${f.plannedLine}\n   State it as a reminder of their own intention, nothing more. Do not ask them to confirm it, do not tell them to stick to it, and do not comment on whether it is a good plan.`
      : ''
  }
${extras}
