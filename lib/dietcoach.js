 ...
 */
function weekPlanBlock(store, cfg) {
  ...
  return [
    ...
    lines,
    'If they ask for a SHOPPING LIST, build it from THIS grid — the ingredients these meals need — grouped the way a shop is laid out. Do not draft from their history while a plan exists. Days marked "nothing planned" need nothing bought and are not a gap to fill.',
  ].join('\n');
}
...
module.exports = { name: 'dietcoach', persona, context, tools, runTool, fmtNutrition, patternBlock, libraryBlock, planBlock, weekPlanBlock };
